...
pub const RayzigCtx = struct {
	...
	pub fn init(allocator: std.mem.Allocator) !Self {
		...
		try materials.addMaterial("light", (try material.Light.create(math.vector.Color3f.init(19 * 2, 12 * 2, 12 * 2), allocator)).material());
		try materials.addMaterial("light2", (try material.Light.create(math.vector.Color3f.init(19 * 2, 19 * 2, 19 * 2), allocator)).material());
		try materials.addMaterial("basic_metal", (try material.Metal.create(math.vector.Color3f.init(1, 1, 1), 0.02, allocator)).material());
		try materials.addMaterial("perfect_metal", (try material.Metal.create(math.vector.Color3f.init(0.9, 0.94, 0.9), 0.0005, allocator)).material());
		try materials.addMaterial("basic_glass", (try material.Glass.create(math.vector.Color3f.init(0.9, 0.94, 0.9), 1.4, 0.001, allocator)).material());
		try materials.addMaterial("basic_glass2", (try material.Glass.create(math.vector.Color3f.init(0.98, 0.24, 0.86), 1.4, 0.1, allocator)).material());
		...
		try world.append((try rt.hittable.Sphere.create(math.vector.Point3f.init(-1, 1, 1.5), 0.1, materials.getMaterial("light2").?, allocator)).hittable());
		try world.append((try rt.hittable.Sphere.create(math.vector.Point3f.init(-1, 1, 1.5), -0.4, materials.getMaterial("basic_glass2").?, allocator)).hittable());
		...
		// try world.append((try rt.hittable.Quad.create(math.vector.Point3f.init(-2.99, 1.1, 2.4), math.vector.Vector3f.init(0, 0, 0.5), math.vector.Vector3f.init(0, -0.5, 0), materials.getMaterial("light").?, allocator)).hittable());
